...
async def test_social_agent():
    ...
    # Analyze sentiment for MNT
    result = await agent.analyze_sentiment("MNT")
    ...
    for platform, data in result['by_platform'].items():
        ...
        print(f"    Score: {data['overall_score']:.3f}")
        print(f"    Distribution: {data['positive_percentage']:.1f}% positive, "
              f"{data['neutral_percentage']:.1f}% neutral, "
              f"{data['negative_percentage']:.1f}% negative")
    ...
    narratives = await agent.get_trending_narratives("MNT")
    print(f"\n🔥 Trending Narratives:")
    for i, narrative in enumerate(narratives[:5], 1):
        print(f"  {i}. {narrative['narrative']} ({narrative['mentions']} mentions)")
