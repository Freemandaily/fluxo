...
class Defillama:
    ...
    async def _fetch_all_mantle_protocols_data(self,session)->dict:
    ...
    # Entry point to Fetch protocols on Mantle chain
    async def fetch_protocol_data(self)->dict:
        async with aiohttp.ClientSession() as session:
            protocols_datas = await self._fetch_all_mantle_protocols_data(session)
    ...
    async def _fetch_yield_protocols(self,session)->dict:
    ...
    # Entry point Fetch yield protocols on Mantle chain [Entry]
    async def fetch_mantle_yield_protocols(self)->dict:
        async with aiohttp.ClientSession() as session:
            protocols_datas = await self._fetch_yield_protocols(session)
            if not protocols_datas:
                raise Exception("No Yield protocols data found")
        
            mantle_yield_protocols = []
            for data in protocols_datas['data']:
                if data['chain'] == 'Mantle':
                    protocol_data = {
